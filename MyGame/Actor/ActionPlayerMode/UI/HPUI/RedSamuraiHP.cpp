...
void RedSamuraiHP::draw(int hp) const
{
	...
	Sprite::GetInstance().DrawPart(SPRITE_ID::REDSAMURAI_HPGAUGE, Vector2(100, Sprite::GetInstance().GetSize(SPRITE_ID::REDSAMURAI_HPFRAME).y), 0, 0,
		static_cast<int>(Sprite::GetInstance().GetSize(SPRITE_ID::REDSAMURAI_HPGAUGE).x / RedSamuraiHPVal * hp), static_cast<int>(Sprite::GetInstance().GetSize(SPRITE_ID::REDSAMURAI_HPGAUGE).y));
}
