...
void ShootingPlayerActor::update(float deltaTime)
{
	...
	if (GetJoypadPOVState(DX_INPUT_PAD1, 0) == 9000 || Keyboard::GetInstance().KeyStateDown(KEYCODE::F))
	{
		if (!alreadyItem && state_ == PlayerState::State::PlayerIdel && GameDataManager::getInstance().GetPlayerDead() == false)
		{
			...
			{
				world_->add_actor(ActorGroup::ItemBoxUI, new_actor<ShootingPlayerItemBox>(world_, RecoverItemCount, AttackItemCount, weak_from_this()));
			}
			alreadyItem = true;
		}
	}
...
void ShootingPlayerActor::update_state(float deltaTime)
{
	//状態遷移
	switch (state_)
	{
...
void ShootingPlayerActor::change_state(PlayerState::State state, int motion)
{
...
void ShootingPlayerActor::PlayerInput()
{
...
void ShootingPlayerActor::IdletoAim()
{
	GunPossible = false;
	state_timer_ += 1.0f;
...
void ShootingPlayerActor::AimtoIdle()
{
	GunPossible = false;
	state_timer_ += 1.0f;
	if (state_timer_ >= mesh_.motion_end_time()) {
...
void ShootingPlayerActor::IdleAiming()
{
	if (GamePad::GetInstance().ButtonTriggerUp(PADBUTTON::NUM5) || Keyboard::GetInstance().KeyTriggerUp(KEYCODE::LSHIFT)) {
		change_state(PlayerState::PlayerAimToIdle, PlayerMotion::Motion::MotionPlayerAimToIdle);
	}
...
void ShootingPlayerActor::Reload()
{
	GunPossible = false;
	state_timer_ += 1.0f;
	if (state_timer_ >= mesh_.motion_end_time())
...
void ShootingPlayerActor::GunPunch()
{
	state_timer_ += 1.0f;
	if (state_timer_ == 60)
	{
...
void ShootingPlayerActor::GunMove(float X, float Y)
{
	//前後左右移動
	velocity_ = Vector3::Zero;
...
void ShootingPlayerActor::Move(float X, float Y)
{
	//前後左右移動
	velocity_ = Vector3::Zero;
	float forward_speed{ 0.0f };
...
void ShootingPlayerActor::Gun()
{
	world_->add_actor(ActorGroup::PlayerBullet, std::make_shared<ShootingPlayerBullet>(world_, Vector3{ position_.x,position_.y + 13.0f,position_.z } +rotation_.Forward() * 4 + rotation_.Right() * 3, AimPos,AttackParam));
	SetRemainGun = SetRemainGun - 1;
}

void ShootingPlayerActor::PlayerGunFire()
{
...
void ShootingPlayerActor::JumpChacker(PlayerState::State state)
{
	switch (state)
	{
...
void ShootingPlayerActor::Damage()
{
	state_timer_ += 1.0f;
	if (state_timer_ >= mesh_.motion_end_time())
	{
...
void ShootingPlayerActor::Tyohatu()
{
...
void ShootingPlayerActor::draw_weapon() const
{
...
void ShootingPlayerActor::Delay()
{
	if (invinciblyCheck) {
		if (hp_ > 0)
		{
...
void ShootingPlayerActor::Hit(Vector3 & dir)
{
