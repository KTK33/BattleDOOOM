...
void ParamUI::draw() const
{
	...
	Sprite::GetInstance().DrawPart(SPRITE_ID::NUMBER, Vector2(static_cast<float>(WINDOW_WIDTH) - GunFreamSize.x*0.95f, static_cast<float>(WINDOW_HEIGHT) - GunFreamSize.y*0.8f), static_cast<int>(NumSize.x / 10.0f) * mRemainGun, 0, static_cast<int>(NumSize.x / 10.0f), static_cast<int>(NumSize.y)); //セットされている弾数(１桁)
	Sprite::GetInstance().Draw(SPRITE_ID::THRASH, Vector2(static_cast<float>(WINDOW_WIDTH) - GunFreamSize.x*0.7f, static_cast<float>(WINDOW_HEIGHT) - GunFreamSize.y*0.8f));//スラッシュ
	int m_P[2] = { mHaveGun / 10,mHaveGun % 10 };
	if (m_P[0] != 0) {
		Sprite::GetInstance().DrawPart(SPRITE_ID::NUMBER, Vector2(static_cast<float>(WINDOW_WIDTH) - GunFreamSize.x*0.5f, static_cast<float>(WINDOW_HEIGHT) - GunFreamSize.y*0.8f), static_cast<int>(NumSize.x / 10.0f) * m_P[0], 0, static_cast<int>(NumSize.x / 10.0f), static_cast<int>(NumSize.y)); //セットされている弾数(１桁)
	}
	Sprite::GetInstance().DrawPart(SPRITE_ID::NUMBER, Vector2(static_cast<float>(WINDOW_WIDTH) - GunFreamSize.x*0.3f, static_cast<float>(WINDOW_HEIGHT) - GunFreamSize.y*0.8f), static_cast<int>(NumSize.x / 10.0f) * m_P[1], 0, static_cast<int>(NumSize.x / 10.0f), static_cast<int>(NumSize.y)); //セットされている弾数(１桁)
