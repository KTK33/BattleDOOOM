...
void AnyUI::draw() const
{
	...
	Sprite::GetInstance().Draw(SPRITE_ID::TEXTFRAME, Vector2((float)WINDOW_WIDTH - Sprite::GetInstance().GetSize(SPRITE_ID::TEXTFRAME).x, 0));

	//’e”
	Vector2 GunFreamSize = Sprite::GetInstance().GetSize(SPRITE_ID::GUN_FREAM);
	Sprite::GetInstance().Draw(SPRITE_ID::GUN_FREAM, Vector2((float)WINDOW_WIDTH - GunFreamSize.x, (int)WINDOW_HEIGHT- GunFreamSize.y));
	...
	Sprite::GetInstance().Draw(SPRITE_ID::THRASH, Vector2((float)WINDOW_WIDTH - GunFreamSize.x*0.7f, (int)WINDOW_HEIGHT- GunFreamSize.y*0.8f));//ƒXƒ‰ƒbƒVƒ…
